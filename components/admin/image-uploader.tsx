...
export function ImageUploader({ productId, initialImages }: ImageUploaderProps) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const { url, error } = await uploadProductImage(fd, productId)
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-gray-400">JPG, PNG, WebP · Max 5MB · Click ★ to set as primary</p>
      ...
      {images.length === 0 ? (
        ...
        >
          ...
          <p className="text-xs text-gray-400">Click to upload the first image</p>
        </div>
      ) : (
